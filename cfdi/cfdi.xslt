...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="cfdi:Comprobante" mode="cfdi:document"/>
    </xsl:template>

    <xsl:template match="cfdi:Comprobante" mode="cfdi:document">
    ...
    <xsl:template name="cfdi-field">
        ...
        <div>
            <xsl:attribute name="class">
                ...
                <xsl:if test="string($class) != ''"><xsl:text> </xsl:text><xsl:value-of select="$class"/></xsl:if>
            </xsl:attribute>
            <span class="cfdi-label"><xsl:value-of select="$label"/></span>
            <span class="cfdi-value"><xsl:choose><xsl:when test="string($value) != ''"><xsl:value-of select="$value"/></xsl:when><xsl:otherwise>-</xsl:otherwise></xsl:choose></span>
        </div>
    </xsl:template>

    <xsl:template name="cfdi-money">
        <xsl:param name="value"/>
        <xsl:choose><xsl:when test="string($value) != ''"><xsl:value-of select="format-number(number($value), '#,##0.00', 'mxn')"/></xsl:when><xsl:otherwise>0.00</xsl:otherwise></xsl:choose>
    </xsl:template>

    <xsl:template name="cfdi-rate">
        <xsl:param name="value"/>
        <xsl:choose><xsl:when test="string($value) != ''"><xsl:value-of select="format-number(number($value), '0.000000')"/></xsl:when><xsl:otherwise>-</xsl:otherwise></xsl:choose>
    </xsl:template>
</xsl:stylesheet>
