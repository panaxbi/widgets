...
xover.listener.on(`beforeFetch?request`, function ({ request, searchParams }) {
	...
	let getNodeValue = function (node) {
		if (node.localName == 'selected' && node.namespaceURI == 'http://panax.io/state') {
			...
			return selection.getAttributeNode("id") || selection.getAttributeNode("key") || selection.getAttributeNode("desc")
		} else {
			return node.value
		}
	}
